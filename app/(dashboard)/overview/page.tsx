...
import { useGetAdminOverviewQuery } from "@/lib/redux/api/dashboardApi";
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="p-4 md:p-8 w-full mx-auto">
            {/* Stats Row */}
            <OverviewStats data={data!.summary} />

            {/* Charts Row */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-4 md:mb-8">
              <div className="lg:col-span-2">
                <MonthlyViewsChart data={data!.monthly_views} />
              </div>
              <div className="lg:col-span-1">
                <LocationDonutChart data={data!.earning_activity} />
              </div>
            </div>

            {/* Recent Users Table */}
            <div className="bg-white rounded-xl shadow-xs border border-[#E9EAEB]">
              <h2 className="text-lg font-semibold text-primary p-5">
                All User
              </h2>
              <UsersTable data={data!.latest_users} />
            </div>
          </div>
        </>
      )}
